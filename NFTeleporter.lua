...
-- debug
	if self.Faction == "Dom" then
		...
		self.Destinations = {
			...
			{ loc = "coordinates -5643.53 -975.21 -660.29 22", destName= "Deradune-Bloodfire Village" },
...
function NFTeleporter:PopulateDestinationList()
	Print ("Populating Destination List")
	...
	if self.Destinations == nil then
		Print ("I died here with no destinations")
		return
	end
...
function NFTeleporter:AddDestinationItem(xmlDocl, nIndex, tItemData)
	...
	if wndListItem == nil then
		Print("Could not load DestinationListItem for some reason.")
		return
	end
...
--	local qty = tonumber(self.wndExtSearchWindow:GetText())
	if self.selectedDestination.loc ~= nil then
		...
		Print("A destination must be selected")
		return
	end
end
